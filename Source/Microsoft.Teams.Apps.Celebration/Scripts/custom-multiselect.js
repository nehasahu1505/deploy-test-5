(function ($) {
    function updateSelectedText(selector, options) {
        setTimeout(function () {
            var selectedOptionsTextElement = selector.closest(".multiselect-native-select").find(".btn-group .multiselect-selected-text");
            if (selectedOptionsTextElement.text() === "None selected") {
                selectedOptionsTextElement.text(options.noneSelectedText);
            } else {
                var modifiedText = "You are celebrating with " + selectedOptionsTextElement.text() + " team(s)";
                selectedOptionsTextElement.text(modifiedText);
            }
        }, 0);
    }
